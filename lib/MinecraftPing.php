...
class MinecraftPing
{
     ...
     * @return bool
     */
    private function getSrv($host){
        $dnsarr = dns_get_record("_minecraft._tcp.".$host , DNS_SRV);
        if(isset($dnsarr[0])){
            return $dnsarr[0];
//            echo ping17($dnsarr[0]["target"],$dnsarr[0]["port"]);
        }else{
//            echo ping17($address[0]);
            return false;
        }
    }

    /**
     * 根据字符串获取host和port，如果有srv也进行处理。
     * @param $str
     * @return array
     */
    public function getHostAndPort($str)
    {
        $address = explode(":", $str);
        if (sizeof($address) > 1) {
            //有冒号，强行host+port
    ...
    public function __construct($str,$timeout = 2){

        list($host,$port) = self::getHostAndPort($str);
        $this->ping($host, $port, $timeout);

    }


    public function ping( $Address, $Port = 25565, $Timeout = 2/*, $ResolveSRV = true */)
    {
...
//        $this->Socket = @fsockopen( $this->ServerAddress, $this->ServerPort, $errno, $errstr, $connectTimeout );
        $this->Socket = @stream_socket_client('tcp://' . $this->ServerAddress . ':' . $this->ServerPort, $errno, $errstr, $connectTimeout);
        if( !$this->Socket )
        {
            $this->Socket = null;
